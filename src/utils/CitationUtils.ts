...
export const getCitationRefs = (input:string, short:boolean):string[] => {
    ...
    return citations == null ? [] : citations.map(citation => normalizeCitationRef(citation.trim(), short) || "").filter(x => x !== "");
}

export const normalizeCitationRef = (citation:string, short:boolean):string | undefined => {
